...
if (isset($_POST['save_student'])) {
    ...
    if ($student_id) {
        $sql = "UPDATE student_details SET roll_no=?, name=?, major_id=? WHERE id=?";
        $db->conn->prepare($sql)->execute([$roll, $name, $major_id, $student_id]);
    } else {
        $sql = "INSERT INTO student_details (roll_no, name, major_id) VALUES (?, ?, ?)";
        $db->conn->prepare($sql)->execute([$roll, $name, $major_id]);
    }
    header("Location: manage_students.php");
    exit();
}
...
<div class="container">
    ...
    <div class="card">
        ...
        <form method="POST" style="display:grid; grid-template-columns: 1fr 2fr 1fr auto; gap:10px;">
    ...
    <div class="card" style="margin-top:20px;">
        <table class="student-table">
            <thead>
                <tr><th>Roll No</th><th>Name</th><th>Major</th><th>Action</th></tr>
            </thead>
            <tbody>
